...
class Player {
  ...
  update() {
    ...
    if (this.isJumping) {
      ...
      if (this.isSliding) {
        ...
        this.y = this.groundY + (50 - GAME_CONFIG.slideHeight); // 슬라이딩 시 Y 위치 조정
      } else {
        this.height = 50; // 기본 높이
        if (!this.isJumping) {
          // 점프 중이 아닐 때만 땅에 고정
  ...
  draw() {
    ...
    // 디노 몸체 (메인 색상)
    // 슬라이딩 중일 때 몸체 위치와 크기 조정
    const bodyY = this.isSliding ? this.y : this.y + 20;
    const bodyHeight = this.isSliding ? GAME_CONFIG.slideHeight - 20 : 25;
    ctx.fillStyle = this.color;
    ctx.fillRect(this.x + 5, bodyY, 30, bodyHeight);

    // 디노 머리
    const headY = this.isSliding ? this.y - 5 : this.y + 10;
    ctx.fillRect(this.x + 35, headY, 15, 15);

    // 디노 목
    const neckY = this.isSliding ? this.y : this.y + 15;
    ctx.fillRect(this.x + 30, neckY, 10, 10);

    // 디노 꼬리
    const tailY = this.isSliding ? this.y + 10 : this.y + 25;
    ctx.fillRect(this.x, tailY, 10, 8);

    // 다리 (슬라이딩 시 다르게 표시)
    if (this.isSliding) {
      // 슬라이딩 시 다리
      ctx.fillRect(this.x + 10, this.y + GAME_CONFIG.slideHeight - 10, 8, 5);
      ctx.fillRect(this.x + 22, this.y + GAME_CONFIG.slideHeight - 10, 8, 5);
    } else {
      // 일반 다리
    ...
    // 눈
    ctx.fillStyle = "white";
    ctx.fillRect(this.x + 37, headY + 2, 3, 3);
    ctx.fillRect(this.x + 42, headY + 2, 3, 3);

    // 눈동자
    ctx.fillStyle = "black";
    ctx.fillRect(this.x + 38, headY + 3, 1, 1);
    ctx.fillRect(this.x + 43, headY + 3, 1, 1);

    // 입
    ctx.fillStyle = "black";
    ctx.fillRect(this.x + 45, headY + 8, 3, 2);
  ...
  getHitbox() {
    // 히트박스 조정: 캐릭터 외형에 더 가깝게, 슬라이딩 시 높이 조정 반영
    if (this.isSliding) {
      return {
        x: this.x + 5,
        y: this.y + (50 - GAME_CONFIG.slideHeight) + 5, // 슬라이딩 시 y 위치 조정
        width: this.width - 10,
        height: GAME_CONFIG.slideHeight - 10,
      };
    } else {
      return {
        x: this.x + 5,
...
class Obstacle {
  constructor(x, type = "cactus", playerIndex = 0) {
    ...
    // Adjust groundY for better collision calculation. Use the actual ground level.
    const groundY = gameAreaY + gameAreaHeight - GAME_CONFIG.ground.height;

    if (type === "cactus") {
      this.width = 25;
      this.height = 70; // Increased height to require double jump (was 60)
      this.y = groundY - this.height;
      this.color = "#228B22";
    } else if (type === "bird") {
      this.width = 40;
      this.height = 25;
      this.y = groundY - 65; // Adjusted bird height to be higher for slide/double jump evasion
      this.color = "#8B4513";
    } else if (type === "lowCactus") {
      this.width = 30;
      this.height = 40; // Slightly increased for better distinction from large cactus (was 35)
      this.y = groundY - this.height;
      this.color = "#32CD32";
    }
  }
